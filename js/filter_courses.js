document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterTable() {
    ...
    Array.from(coursesTable.getElementsByTagName("tr")).forEach(function (
      row,
      index
    ) {
      if (index === 0) return; // Skip the header row
      const courseNameCell = row.getElementsByTagName("td")[1];
      const professorNameCell = row.getElementsByTagName("td")[2];
      ...
      if (courseNameCell && professorNameCell) {
        ...
        if (
          courseName.includes(courseNameFilter) &&
          professorName.includes(professorNameFilter)
        ) {
          row.style.display = "";
        } else {
